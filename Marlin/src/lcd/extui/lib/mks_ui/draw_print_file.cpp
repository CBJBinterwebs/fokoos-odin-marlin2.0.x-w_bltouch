...
void lv_draw_print_file(void) {
  /* lv_obj_t *e1, *bed;
    ...
    lv_obj_set_pos(e1, 250, 7);
    sprintf_P(buf, PSTR("e: %d"), thermalManager.wholeDegHotend(0));
    lv_label_set_text(e1, buf);
  #endif
  #if HAS_HEATED_BED
    bed = lv_label_create_empty(scr);
    lv_obj_set_pos(bed, 295, 7);
    sprintf_P(buf, PSTR("b: %d"), thermalManager.wholeDegBed());
